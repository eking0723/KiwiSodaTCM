## **USER-002:** User Testing - Promote User to Project Manager  

> **Summary:** Verify that user is promoted as project manager.  <br>

**Preconditions:** 
- Device must be connected to the internet
- At least one `Viewer` user type exists in the system.
...
Scenario 1 
 ...
 |  5 |Click `Promote to PM` option      | Verify that a confirmation dialouge appears on the screen | 
 |  6 |Click `Confirm` button      | Verify that the `Confirm` button changed in to "processing" with loading animation   | 
 |  7 |Wait until the processing animation finishes | Verify that the role in the `Role` column has been updated to `Project Manager` successfully|
